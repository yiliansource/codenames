...
import { Card, TeamColour, Player, GamePhase } from "../../../shared/codenames";
...
export const TeamComponent: FunctionComponent<TeamProps> = ({ players, colour, onSwitch }) => {
    ...
    const teamCards = (gameContext.cards || []).filter(c => c.colour === colour);
    let totalCount = teamCards.length;
    let cardsFound = teamCards.filter(c => c.isConsumed).length;

    return <div className={"relative m-auto transition-opacity " + (gameContext.phase !== GamePhase.Round || gameContext.inTurn === colour ? "opacity-100" : "opacity-50")}>
        ...
        <div style={{ minWidth: "14rem" }} className="relative rounded-lg shadow-md bg-gray-100 px-8 py-8">
            ...
            <ul className="block divide-y divide-gray-300">
                {players.map((p, i) => <li key={i} className={"px-4 py-1 " + (p.id === userContext.id ? "font-bold" : "")}>
            ...
            { gameContext.phase === GamePhase.Lobby && userContext.team !== colour
