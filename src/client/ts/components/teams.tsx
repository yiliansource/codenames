...
import { createFormatContainer, formatPlayer, formatTeam } from '../formatting';
...
export const TeamComponent: FunctionComponent<TeamProps> = ({ players, colour, onSwitch }) => {
    ...
    return <div className={"relative m-auto transition-opacity " + (game.phase !== GamePhase.Round || game.inTurn === colour ? "opacity-100" : "opacity-50")}>
        ...
        <div style={{ minWidth: "14rem" }} className="relative rounded-lg shadow-md bg-gray-100 px-8 py-8">
            <h1 className="text-center">{createFormatContainer(formatTeam(colour))}</h1>
            
            {/* Render a trophy for each victory the team has achieved. */}
            { roundsWon > 0
                ? <div className="text-center">
                    { Array(roundsWon).fill('').map((v, i) => <span key={i} title="Sweet victory!" className="text-yellow-400 mx-1 text-base"><i className="fas fa-trophy"></i></span>) }
                </div>
                : null
            }
            ...
            <ul className="block divide-y divide-gray-300 mt-4">
                { players.slice().sort((a, b) => (Number(b.isGameMaster) - Number(a.isGameMaster)) || a.name.localeCompare(b.name))
                    .map((p, i) => <li key={i} className={"px-4 py-1 " + (p.id === user.id ? "font-bold" : "") + " " + (p.isReconnecting ? "opacity-40": "")}>
                        {createFormatContainer(formatPlayer(p, { colourName: false }))}
                    </li>) }
            </ul>
            ...
            { game.phase !== GamePhase.Lobby && hints.length > 0
                ? <ul className="block divide-y divide-gray-300 mt-5">
                    { hints.map((h, i) => <li key={i} className="px-4 py-1 text-gray-500">
